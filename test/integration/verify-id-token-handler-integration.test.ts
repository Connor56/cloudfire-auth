...
describe.skipIf(doNotRunIntegrationTests)("Verify ID Token Handler Integration Tests", async () => {
  ...
  const userEmail = "test-user-123@example.com";
  const userDisplayName = "test-user-123";

  // Create a new user
  const tokenData = await addANewUserWithSignUp(oauth2TokenFromKV, userEmail, userDisplayName);

  const signInData = await signInWithPassword(oauth2TokenFromKV, userEmail);

  await new Promise((resolve) => setTimeout(resolve, 500));

  console.log("signInData", signInData);

  const userData = await getUserByEmail(signInData.email, oauth2TokenFromKV);

  it("The user data should be retrieved", async () => {
    const user = userData.users[0];
    expect(userData).toBeDefined();
    expect(user.email).toBe(userEmail);
    expect(user.displayName).toBe(userDisplayName);
    expect(user.localId).toBe(tokenData.localId);
  }, 10000);
  ...
  it("Should reject tokens after user refresh tokens are revoked", async () => {
    ...
    console.log("revoked user refresh tokens");
...
/**
 * Only used to debug token validation issues. Uncomment if you want to test a
 * failing token and run it explicitly with the line number of the test.
 */
